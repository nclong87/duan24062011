...
<div id="content" style="width:100%">
	...
	foreach($lstArticlesOlder as $a) {
		$alink = BASE_PATH."/tintuc/view/".$a["tintuc"]["id"]."/".$a["tintuc"]["alias"];
		?>
		<li>
		<a class="link" href="<?php echo $alink ?>"><?php echo $a["tintuc"]["title"] ?></a> 
		</li>
		<?php
	}
	?>
	...
	<center>
	<form id="sendComment">
	<input type="hidden" name="tintuc_id" value="<?php echo $tintuc["tintuc"]["id"]?>"/>
...
<script>
	...
	function loadComments(page) {
		...
		$.ajax({
			type : "GET",
			cache: false,
			url: url("/tintuc/comments/<?php echo $tintuc["tintuc"]['id']?>/"+page),
	...
	function doDeleteComment(_this,id) {
		...
		$.ajax({
			type : "GET",
			cache: false,
			url : url("/tintuc/doDeleteComment&comment_id="+id),
			success : function(data){	
				unblock("#datagrid");
				if(data == AJAX_DONE) {
